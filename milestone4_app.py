...
def _build_report_pdf(report_text: str, report_date: str) -> bytes:
    """Build a professional PDF from the LLM-generated report text."""
    ...
    pdf.set_xy(15, 14)
    pdf.cell(180, 8, "GFI Weekly Threat Intelligence Report")
    pdf.set_font("Helvetica", "", 9)
    pdf.set_xy(15, 23)
    pdf.cell(180, 5, f"Generated: {report_date}  |  Classification: TLP:AMBER  |  Distribution: CISO, SOC, Compliance")

    # ── Gold accent line ──
    pdf.set_draw_color(201, 160, 23)
    ...
    for raw_line in clean.split('\n'):
        ...
        is_header = (line.isupper() and len(line) > 5) or \
                    re.match(r'^\d+\.\s+[A-Z]', line)

        pdf.set_x(left_margin)  # reset x before every line

        if line == '---':
            pdf.set_draw_color(226, 232, 240)
            pdf.line(left_margin, pdf.get_y() + 1, 200, pdf.get_y() + 1)
            pdf.ln(5)
        elif is_header:
    ...
    pdf.multi_cell(190, 4, "This report was automatically generated by the GFI CTI Platform using Google Gemini Flash LLM.\n"
                           "Validate all IOCs before deploying to production controls.\n"
...
- Professional tone suitable for distribution to C-suite and technical staff"""
            ...
            if report_html:
                ...
                _pdf_bytes = _build_report_pdf(report_html, date.today().strftime('%B %d, %Y'))

                st.download_button(
                    "⬇️ Download Report (PDF)",
                    _pdf_bytes,
                    f"GFI_Weekly_Report_{date.today().isoformat()}.pdf",
                    "application/pdf",
